...
const [
  ...
  // Accept structured-syntax-suffix JSON content types (e.g. LiveKit sends
  // `application/webhook+json`). The built-in parser only handles
  // `application/json`, so without this such payloads 415 before the handler.
  // The regex intentionally matches `*+json` only, to avoid clashing with the
  // already-registered `application/json` parser.
